...
void auditorio()
{
    ...
    */
    ...
    do
    {
        ...
        printf("\n\n    (1) >>> Salvar o projeto ?");
        printf(  "\n    (2) >>> Redimencionare  o projeto atual ?");

        printf("\n\n    (3) >>> Adicionar corredor vertical ?");
        printf(  "\n    (4) >>> Adicionar corredor horizontal ?");

        printf("\n\n    (5) >>> Excluir linha vertical ?");
        printf(  "\n    (6) >>> Excluir coluna horizontal ?");

        printf("\n\n    (7) >>> Abandinar o projeto ?");
        ...
        switch (opcao)
        {
            ...
            case 5:
                ...
                CRIAR_MATRIZ
                excluirCorredor(&deleteFila, &cVertical, &numCorredorVer);
...
/*
    Perguntar qual fila vc que deletar ?
    Digita a fila 
    Deleta ...

    Receber uma var pra fila a ser deletada.
    Vetor
    tam do vetor
*/
            ...
            /*
                Excluir o num
                Jogar os outros um posição para traz
                Decrementar os numeros
            */

void excluirCorredor(int *deleteFila, int *vetor, int *tamDoVetor)
{
    printf("Qual Fila vc quer deletar ?");
    scanf("%d%*c", &*deleteFila);
    fflush(stdin);

    for (int z = 0; z < *tamDoVetor; z++)
    {
        if (vetor[z] == *deleteFila)
        {
            for ( int y = z; y < *tamDoVetor; z++ )
            {
                vetor[y] = vetor[y+1];
            }
